/// @file matrix.h
/// @brief The declaration of 3 kinds of matrices: double, complex-double,
/// and the matrix of complex matrix (4th order tensor)
///
...
public:
    ...
    // ComplexMatrixMatrix could access to ComplexMatrix for its functions
    friend class ComplexMatrixMatrix;
};

/// class for density matrix;
/// outer matrix is grid point (Ri, Pj),
/// and inner matrix is density matrix (a pes, b pes);
/// using dynamic memory management in C++17 for memory allocation
/// @see ComplexMatrix
class ComplexMatrixMatrix
{
private:
    const int length; ///< the number of elements in a row/column
    const int nelements; ///< the total number of elements, i.e. length^2
    ComplexMatrix** content; ///< the memory to save the matrix content
    allocator<ComplexMatrix> MatrixAllocator; ///< the allocator to allocate the memory for ComplexMatrix
public:
    /// @brief constructor
    /// @param OuterLength the length of this matrix over matrix, i.e. the number of grids
    /// @param InnerLength the length for ComplexMatrix
    ComplexMatrixMatrix(const int OuterLength, const int InnerLength);
    /// @brief destructor;
    ~ComplexMatrixMatrix(void);
    /// @brief overload operator[]
    /// @param idx the row number
    /// @return a pointer to the first ComplexMatrix of the row
    ...
    /// @brief output
    /// @param os an ostream object (could be ofstream/osstream), the output stream
    /// @param rho the grided density matrix
    /// @return the ostream object in the parameter list after output
